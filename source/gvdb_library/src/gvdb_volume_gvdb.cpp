...
Vector3DI VolumeGVDB::getVersion() { return Vector3DI(MAJOR_VERSION, MINOR_VERSION, 0); }
...
void VolumeGVDB::LoadFunction(int fid, std::string func, int mid, std::string ptx) {
  ...
  if (cuFunc[fid] == (CUfunction)-1)
    cudaCheck(cuModuleGetFunction(&cuFunc[fid], cuModule[mid], cfn), "VolumeGVDB", "LoadFunction",
...
void VolumeGVDB::SetCudaDevice(int devid, CUcontext ctx) {
  ...
  // Boolean Operations
  LoadFunction(FUNC_BOOLEAN_UNION, "gvdbBooleanUnion", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
  LoadFunction(FUNC_BOOLEAN_INTERSECT, "gvdbBooleanIntersection", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
  LoadFunction(FUNC_BOOLEAN_DIFFERENCE, "gvdbBooleanDifference", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
...
void VolumeGVDB::SetModule() { SetModule(cuModule[MODL_PRIMARY]); }
...
void VolumeGVDB::SetModule(CUmodule module) {
  ...
  size_t len = 0;
  cudaCheck(cuModuleGetGlobal(&cuScnInfo, &len, module, "scn"), "VolumeGVDB", "SetModule", "cuModuleGetGlobal",
            "cuScnInfo", mbDebug);

  cudaCheck(cuModuleGetGlobal(&cuXform, &len, module, "cxform"), "VolumeGVDB", "SetModule", "cuModuleGetGlobal",
            "cuXform", mbDebug);
  cudaCheck(cuModuleGetGlobal(&cuDebug, &len, module, "cdebug"), "VolumeGVDB", "SetModule", "cuModuleGetGlobal",
            "cuDebug", mbDebug);
  ...
  for (int n = 0; n < num_grids; n++) {
    ...
    for (int chan = 0; chan < num_chan; chan++) {
      ...
      for (int z = 0; z < axisres.z; z++) {
        ...
        mPool->AtlasWriteSlice(chan, z, static_cast<int>(slice.size), slice.gpu,
...
void VolumeGVDB::ClearAtlasAccess() {
  ...
  for (int chan = 0; chan < num_chan; chan++) {
    if (mVDBInfo.volIn[chan] != ID_UNDEFL) {
      cudaCheck(cuTexObjectDestroy(mVDBInfo.volIn[chan]), "VolumeGVDB", "ClearAtlasAccess", "cuTexObjectDestroy",
                "volIn", mbDebug);
      mVDBInfo.volIn[chan] = ID_UNDEFL;
    }
    if (mVDBInfo.volOut[chan] != ID_UNDEFL) {
      cudaCheck(cuSurfObjectDestroy(mVDBInfo.volOut[chan]), "VolumeGVDB", "ClearAtlasAccess", "cuSurfObjectDestroy",
                "volOut", mbDebug);
      mVDBInfo.volIn[chan] = ID_UNDEFL;
    }
  }
...
void VolumeGVDB::SetupAtlasAccess() {
  ...
  for (int chan = 0; chan < num_chan; chan++) {
    ...
    if (atlas.grsc != 0x0) {
      cudaCheck(cuGraphicsMapResources(1, &atlas.grsc, mStream), "VolumeGVDB", "SetupAtlasAccess",
                "cuGraphicsMapResource", chanmsg, mbDebug);
      cudaCheck(cuGraphicsSubResourceGetMappedArray(&atlas.garray, atlas.grsc, 0, 0), "VolumeGVDB", "SetupAtlasAccess",
                "cuGraphicsSubResourceGetMappedArray", chanmsg, mbDebug);
    }
    ...
    if (mVDBInfo.volIn[chan] != ID_UNDEFL) {
      cudaCheck(cuTexObjectDestroy(mVDBInfo.volIn[chan]), "VolumeGVDB", "SetupAtlasAccess", "cuTexObjectDestroy",
                chanmsg, mbDebug);
      mVDBInfo.volIn[chan] = ID_UNDEFL;
    }
    if (mVDBInfo.volOut[chan] != ID_UNDEFL) {
      cudaCheck(cuSurfObjectDestroy(mVDBInfo.volOut[chan]), "VolumeGVDB", "SetupAtlasAccess", "cuSurfObjectDestroy",
                chanmsg, mbDebug);
      mVDBInfo.volOut[chan] = ID_UNDEFL;
    }

    cudaCheck(cuTexObjectCreate(&mVDBInfo.volIn[chan], &resDesc, &texDesc, NULL), "VolumeGVDB", "SetupAtlasAccess",
              "cuTexObjectCreate", chanmsg, mbDebug);
    cudaCheck(cuSurfObjectCreate(&mVDBInfo.volOut[chan], &resDesc), "VolumeGVDB", "SetupAtlasAccess",
              "cuSurfObjectCreate", chanmsg, mbDebug);

    if (atlas.grsc != 0x0)
      cudaCheck(cuGraphicsUnmapResources(1, &atlas.grsc, mStream), "VolumeGVDB", "SetupAtlasAccess",
                "cuGraphicsUnmapResources", chanmsg, mbDebug);
  }
...
void VolumeGVDB::FindUniqueBrick(int pNumPnts, int pLevDepth, int& pNumUniqueBrick) {
  ...
  void* argsFindUnique[5] = {&numPnts, &mAux[AUX_SORTED_LEVXYZ].gpu, &mAux[AUX_MARKER].gpu, &mAux[AUX_UNIQUE_CNT].gpu,
                             &mAux[AUX_LEVEL_CNT].gpu};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_FIND_UNIQUE], pblks, 1, 1, threads, 1, 1, 0, NULL, argsFindUnique, NULL),
            "VolumeGVDB", "FindUniqueBrick", "cuLaunch", "FUNC_FIND_UNIQUE", mbDebug);

  cudaCheck(cuMemcpyDtoH(&pNumUniqueBrick, mAux[AUX_UNIQUE_CNT].gpu, sizeof(int)), "VolumeGVDB", "FindUniqueBrick",
            "cuMemcpyDtoH", "AUX_UNIQUE_CNT", mbDebug);
  ...
  void* argsCompactUnique[5] = {&numPnts, &mAux[AUX_SORTED_LEVXYZ].gpu, &mAux[AUX_MARKER].gpu,
                                &mAux[AUX_MARKER_PRESUM].gpu, &mAux[AUX_UNIQUE_LEVXYZ].gpu};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_COMPACT_UNIQUE], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCompactUnique, NULL),
            "VolumeGVDB", "FindUniqueBrick", "cuLaunch", "FUNC_COMPACT_UNIQUE", mbDebug);
...
void VolumeGVDB::ActivateExtraBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev,
                                        Vector3DI pRootPos) {
  ...
  cudaCheck(
      cuLaunchKernel(cuFunc[FUNC_CALC_EXTRA_BRICK_ID], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCalcExtraLevXYZ, NULL),
      "VolumeGVDB", "ActivateExtraBricksGPU", "cuLaunch", "FUNC_CALC_EXTRA_BRICK_ID", mbDebug);
  ...
  cudaCheck(cuMemcpyDtoH(extraLevCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int)), "VolumeGVDB",
            "ActivateExtraBricksGPU", "cuMemcpyDtoH", "AUX_LEVEL_CNT", mbDebug);

  unsigned short* extraUniBricks = (unsigned short*)malloc(numUniqueExtraBrick * 4 * sizeof(unsigned short));
  cudaCheck(cuMemcpyDtoH(extraUniBricks, mAux[AUX_UNIQUE_LEVXYZ].gpu, numUniqueExtraBrick * 4 * sizeof(unsigned short)),
            "VolumeGVDB", "ActivateExtraBricksGPU", "cuMemcpyDtoH", "AUX_UNIQUE_LEVXYZ", mbDebug);
  ...
  for (int lev = 0; lev < pRootLev; lev++) {
    ...
    for (int n = extraLevPrefixSum; n < extraLevCnt[lev] + extraLevPrefixSum; n++) {
      ...
      SetupNode(nodeID, lev,
                Vector3DI(extraUniBricks[n * 4 + 2] * brickRange.x, extraUniBricks[n * 4 + 1] * brickRange.y,
  ...
  for (int lev = pRootLev - 1; lev >= 0; lev--) {
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_LINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsLink, NULL),
              "VolumeGVDB", "ActivateExtraBricksGPU", "cuLaunch", "FUNC_LINK_BRICKS", mbDebug);
  }
  PERF_POP();
...
void VolumeGVDB::ActivateIncreBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos,
                                        bool bAccum) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_CALC_INCRE_EXTRA_BRICK_ID], pblks, 1, 1, threads, 1, 1, 0, NULL,
                           argsCalcExtraLevXYZ, NULL),
  ...
  cudaCheck(cuMemcpyDtoH(extraLevCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int)), "VolumeGVDB",
            "ActivateIncreBricksGPU", "cuMemcpyDtoH", "AUX_LEVEL_CNT", mbDebug);

  unsigned short* extraUniBricks = new unsigned short[numUniqueExtraBrick * 4];
  cudaCheck(cuMemcpyDtoH(extraUniBricks, mAux[AUX_UNIQUE_LEVXYZ].gpu, numUniqueExtraBrick * 4 * sizeof(unsigned short)),
            "VolumeGVDB", "ActivateIncreBricksGPU", "cuMemcpyDtoH", "AUX_UNIQUE_LEVXYZ", mbDebug);
  ...
  for (int lev = pRootLev - 1; lev >= 0; lev--) {
    void* argsLink[2] = {&cuVDBInfo, &lev};
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_LINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsLink, NULL),
              "VolumeGVDB", "ActivateIncreBricksGPU", "cuLaunch", "FUNC_LINK_BRICKS", mbDebug);
  }
  PERF_POP();
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_DELINK_LEAF_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsDelinkLeaf, NULL),
            "VolumeGVDB", "ActivateIncreBricksGPU", "cuLaunch", "FUNC_DELINK_LEAF_BRICKS", mbDebug);
  for (int lev = 1; lev < pRootLev; lev++) {
    void* argsDelink[2] = {&cuVDBInfo, &lev};
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_DELINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsDelink, NULL),
              "VolumeGVDB", "ActivateIncreBricksGPU", "cuLaunch", "FUNC_DELINK_BRICKS", mbDebug);
  }
  PERF_POP();
  ...
  for (int ni = 0; ni < mPool->getPoolTotalCnt(0, 0); ni++)
    if (!getNode(0, 0, ni)->mFlags) usedNum--;
...
void VolumeGVDB::ActivateBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos) {
  ...
  for (int lev = 0; lev < pRootLev; lev++) range_res[lev] = getRange(lev).x;
  ...
  //////////////////////////////////////////////////////////////////////////
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_CALC_BRICK_ID], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCalcLevXYZ, NULL),
            "VolumeGVDB", "ActivateBricksGPU", "cuLaunch", "FUNC_CALC_BRICK_ID", mbDebug);
  ...
  cudaCheck(cuMemcpyDtoH(levCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int)), "VolumeGVDB", "ActivateBricksGPU",
            "cuMemcpyDtoH", "AUX_LEVEL_CNT", mbDebug);

  unsigned short* uniBricks = new unsigned short[numUniqueBrick * 4];
  cudaCheck(cuMemcpyDtoH(uniBricks, mAux[AUX_UNIQUE_LEVXYZ].gpu, numUniqueBrick * 4 * sizeof(unsigned short)),
            "VolumeGVDB", "ActivateBricksGPU", "cuMemcpyDtoH", "AUX_UNIQUE_LEVXYZ", mbDebug);
  ...
  for (int lev = 0; lev < pRootLev; lev++) {
    ...
    for (int n = levPrefixSum; n < levCnt[lev] + levPrefixSum; n++) {
      uint64 nodeID = mPool->PoolAlloc(0, lev, true);
      SetupNode(nodeID, lev,
                Vector3DI(uniBricks[n * 4 + 2] * brickRange.x, uniBricks[n * 4 + 1] * brickRange.y,
  ...
  for (int lev = pRootLev - 1; lev >= 0; lev--) {
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_LINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsLink, NULL),
              "VolumeGVDB", "ActivateBricksGPU", "cuLaunch", "FUNC_LINK_BRICKS", mbDebug);
  }
  PERF_POP();
...
void VolumeGVDB::FindActivBricks(int pLev, int pRootlev, int pNumPnts, Vector3DF pOrig, Vector3DI pRootPos) {
  ...
  cudaCheck(cuMemcpyDtoH(brickIdx, mAux[AUX_PBRICKDX].gpu, pNumPnts * sizeof(int)), "VolumeGVDB", "ActivateBricksGPU",
            "cuMemcpyDtoH", "AUX_PBRICKDX", mbDebug);

  Vector3DF* sortedPos = new Vector3DF[pNumPnts];
  Vector3DF* pos = new Vector3DF[pNumPnts];
  cudaCheck(cuMemcpyDtoH(pos, mAux[AUX_PNTPOS].gpu, pNumPnts * sizeof(Vector3DF)), "VolumeGVDB", "ActivateBricksGPU",
            "cuMemcpyDtoH", "AUX_PNTPOS", mbDebug);
  ...
  for (int n = 0; n < numActivatingBrick; n++) {
    ...
    SetupNode(nodeID, pLev - 1,
              Vector3DI(activatingBrickIdx[n] % dim * brickRange.x, activatingBrickIdx[n] / dim % dim * brickRange.y,
...
uchar VolumeGVDB::GetChannelType(uchar channel) { return mPool->getAtlas(channel).type; }
...
void VolumeGVDB::ClearAllChannels() {
  ...
  PERF_PUSH("Clear All");
  for (int n = 0; n < mPool->getNumAtlas(); n++) ClearChannel(n);
...
void VolumeGVDB::SaveVBX(const std::string fname) {
  ...
  for (int n = 0; n < num_grids; n++) {
    ...
    for (int chan = 0; chan < num_chan; chan++) {
      ...
      for (int z = 0; z < axisres.z; z++) {
        mPool->AtlasRetrieveSlice(chan, z, static_cast<int>(slice.size), slice.gpu,
                                  (uchar*)slice.cpu);  // transfer from GPU, directly into CPU atlas
...
bool VolumeGVDB::LoadVDBInternal(openvdb::GridBase::Ptr& baseGrid) {
  ...
  const Vector3DF voxelsize =
      Vector3DF(static_cast<float>(grid->voxelSize().x()), static_cast<float>(grid->voxelSize().y()),
  ...
  for (leaf_max = 0; iterator.test();) {
    ...
    if (p0.x > vclipmin.x && p0.y > vclipmin.y && p0.z > vclipmin.z && p0.x < vclipmax.x && p0.y < vclipmax.y &&
        p0.z < vclipmax.z) {
      if (leaf_max == 0) {
        mVoxMin = p0;
        mVoxMax = p0;
  ...
  for (int n = leaf_max = 0; iterator.test(); n++) {
    ...
    if (p0.x > vclipmin.x && p0.y > vclipmin.y && p0.z > vclipmin.z && p0.x < vclipmax.x && p0.y < vclipmax.y &&
        p0.z < vclipmax.z) {  // accept condition
  ...
  PERF_POP();
  if (mValMin != FLT_MAX || mValMax != FLT_MIN) verbosef("\n    Value Range: %f %f\n", mValMin, mValMax);
...
struct Activator {
  ...
  static inline void op(const GridValueAllIter& iter) {
    if (iter.getValue() != 0.0) iter.setActiveState(true);
  }
};
...
void VolumeGVDB::SaveVDB(std::string fname) {
  ...
  if (mPool->getAtlas(0).type != T_FLOAT) {
    gprintf(
        "ERROR: Unable to save .vdb file. Atlas channel 0 must have "
...
void VolumeGVDB::AddPath(const char* path) { mScene->AddPath(path); }
bool VolumeGVDB::FindFile(std::string fname, char* path) { return mScene->FindFile(fname, path); }
...
void VolumeGVDB::Initialize() {
  ...
  mHasObstacle = false;
  cuSecondVDBInfo = 0;  // Initialize for boolean operations
...
void VolumeGVDB::Configure(int levs, int* r, int* numcnt, bool use_masks) {
  ...
  for (int n = 1; n < levs; n++) mPool->PoolCreate(1, n, sizeof(uint64) * getVoxCnt(n), maxcnt[n], true);
...
slong VolumeGVDB::AllocateNode(int lev) { return mPool->PoolAlloc(0, lev, true); }
...
void VolumeGVDB::UpdateAtlas() {
  ...
  if (usedLeafcnt > amax) {
    ...
    for (int n = 0; n < mPool->getNumAtlas(); n++) mPool->AtlasResize(n, usedLeafcnt);
  ...
  for (uint64 n = 0; n < totalLeafcnt; n++) {
    ...
    if (node->mValue.x > atlasmax.x || node->mValue.y > atlasmax.y || node->mValue.z > atlasmax.z) {
      gprintf("ERROR: Node value exceeds atlas res. node: %d, val: %d %d %d, atlas: %d %d %d\n", n, node->mValue.x,
              node->mValue.y, node->mValue.z, atlasres.x, atlasres.y, atlasres.z);
      gerror();
    }
...
slong VolumeGVDB::Reparent(int lev, slong prevroot_id, Vector3DI pos, bool& bNew) {
  ...
  if (newroot_id == ID_UNDEFL) return ID_UNDEFL;
...
#ifdef USE_BITMASKS
  if (curr->mLev > 0) {
    gprintf("%*s L%d #%d, Bit: %d, Pos: %d %d %d, Mask: %s\n", (5 - curr->mLev) * 2, "", (int)curr->mLev, ndx, b,
            curr->mPos.x, curr->mPos.y, curr->mPos.z, binaryStr(*curr->getMask()));
    ...
    gprintf("%*s L%d #%d, Bit: %d, Pos: %d %d %d, Atlas: %d %d %d\n", (5 - curr->mLev) * 2, "", (int)curr->mLev, ndx, b,
            curr->mPos.x, curr->mPos.y, curr->mPos.z, curr->mValue.x, curr->mValue.y, curr->mValue.z);
  }
#endif
}
...
Node* VolumeGVDB::getChild(Node* curr, uint ndx) {
  ...
  uint64* clist = mPool->PoolData64(curr->mChildList);
  uint64* cnext =
      (uint64*)mPool->PoolData(ElemGrp(curr->mChildList), ElemLev(curr->mChildList), ElemNdx(curr->mChildList) + 1);
  uint64 ch;
#ifdef USE_BITMASKS
  ch = *(clist + ndx);
#else
...
void VolumeGVDB::AuxGeometryMap(Model* model, int vertaux, int elemaux) {
  ...
  cudaCheck(cuGraphicsGLRegisterBuffer(&vaux->grsc, model->vertBufferID, CU_GRAPHICS_MAP_RESOURCE_FLAGS_READ_ONLY),
            "VolumeGVDB", "AuxGeometryMap", "cuGraphicsGLRegisterBuffer", "vaux", mbDebug);
  cudaCheck(cuGraphicsMapResources(1, &vaux->grsc, 0), "VolumeGVDB", "AuxGeometryMap", "cuGraphicsMapResources", "vaux",
            mbDebug);
  cudaCheck(cuGraphicsResourceGetMappedPointer(&vaux->gpu, &vsize, vaux->grsc), "VolumeGVDB", "AuxGeometryMap",
            "cuGraphicsResourceGetMappedPointer ", "vaux", mbDebug);
  vaux->lastEle = model->vertCount;
  vaux->usedNum = model->vertCount;
  vaux->stride = model->vertStride;
  vaux->size = vsize;

  cudaCheck(cuGraphicsGLRegisterBuffer(&eaux->grsc, model->elemBufferID, CU_GRAPHICS_MAP_RESOURCE_FLAGS_READ_ONLY),
            "VolumeGVDB", "AuxGeometryMap", "cuGraphicsGLRegisterBuffer", "eaux", mbDebug);
  cudaCheck(cuGraphicsMapResources(1, &eaux->grsc, 0), "VolumeGVDB", "AuxGeometryMap", "cuGraphicsMapResources", "eaux",
            mbDebug);
  cudaCheck(cuGraphicsResourceGetMappedPointer(&eaux->gpu, &esize, eaux->grsc), "VolumeGVDB", "AuxGeometryMap",
            "cuGraphicsResourceGetMappedPointer", "eaux", mbDebug);
...
void VolumeGVDB::AuxGeometryUnmap(Model* model, int vertaux, int elemaux) {
  ...
  cudaCheck(cuGraphicsUnmapResources(1, &vaux->grsc, 0), "VolumeGVDB", "AuxGeometryUnmap", "cuGraphicsUnmapResources",
            "vaux", mbDebug);
  cudaCheck(cuGraphicsUnmapResources(1, &eaux->grsc, 0), "VolumeGVDB", "AuxGeometryUnmap", "cuGraphicsUnmapResources",
            "eaux", mbDebug);
...
int VolumeGVDB::VoxelizeNode(Node* node, uchar chan, Matrix4F* xform, float bdiv, float val_surf, float val_inside,
                             float vthresh) {
  int cnt = 0;
  ...
  cudaCheck(
      cuLaunchKernel(cuFunc[FUNC_VOXELIZE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
...
void VolumeGVDB::SolidVoxelize(uchar chan, Model* model, Matrix4F* xform, float val_surf, float val_inside,
                               float vthresh) {
  PUSH_CTX
  ...
  cudaCheck(cuMemcpyHtoD(cuXform, xform->GetDataF(), sizeof(float) * 16), "VolumeGVDB", "SolidVoxelize", "cuMemcpyHtoD",
            "cuXform", mbDebug);  // Send transform
...
Vector3DI VolumeGVDB::InsertTriangles(Model* model, Matrix4F* xform, float& ydiv) {
  ...
  void* args[7] = {
      &ydiv, &ybins, &mAux[AUX_GRIDCNT].gpu, &vcnt, &ecnt, &mAux[AUX_VERTEX_BUF].gpu, &mAux[AUX_ELEM_BUF].gpu};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_INSERT_TRIS], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "InsertTriangles", "cuLaunch", "FUNC_INSERT_TRIS", mbDebug);
  ...
  // Reset grid counts
  cudaCheck(cuMemsetD8(mAux[AUX_GRIDCNT].gpu, 0, sizeof(uint) * ybins), "VolumeGVDB", "InsertTriangles", "cuMemsetD8",
            "AUX_GRIDCNT", mbDebug);
  ...
  void* args2[10] = {&ydiv, &ybins, &mAux[AUX_GRIDCNT].gpu,    &mAux[AUX_GRIDOFF].gpu, &tri_cnt, &mAux[AUX_TRI_BUF].gpu,
                     &vcnt, &ecnt,  &mAux[AUX_VERTEX_BUF].gpu, &mAux[AUX_ELEM_BUF].gpu};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_SORT_TRIS], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args2, NULL), "VolumeGVDB",
            "InsertTriangles", "cuLaunch", "FUNC_SORT_TRIS", mbDebug);
          ...
          for (int y0 = 0; y0 < res1; y0++) {
            for (int x0 = 0; x0 < res1; x0++) {
              ...
              if (vpix[0] > 0) {
                ...
                c[1] = c[2] = c[3] = c[4] = c[5] = c[6] = 0;
                vpix[1] =
                    static_cast<uchar>((x0 == res1 - 1) ? (++c[1] == 1) : *(vdat + (z0 * res1 + y0) * res1 + (x0 + 1)));
                vpix[2] =
                    static_cast<uchar>((y0 == res1 - 1) ? (++c[2] == 1) : *(vdat + (z0 * res1 + (y0 + 1)) * res1 + x0));
                vpix[3] =
                    static_cast<uchar>((z0 == res1 - 1) ? (++c[3] == 1) : *(vdat + ((z0 + 1) * res1 + y0) * res1 + x0));
...
void VolumeGVDB::getUsage(Vector3DF& ext, Vector3DF& vox, Vector3DF& used, Vector3DF& free) {
  ...
  if (mPool->getNumAtlas() > 0) {
    ...
    vox = Vector3DF(abrk, abrk * leafdim * leafdim * leafdim / 1000000.0f,
...
void VolumeGVDB::Measure(bool bPrint) {
  ...
  if (bPrint) {
    ...
    if (mPool->getNumAtlas() > 0) {
      ...
      gprintf("   Atlas Res:     %d x %d x %d  LeafCnt: %d x %d x %d  LeafDim: %d^3\n", axisres.x, axisres.y, axisres.z,
              axiscnt.x, axiscnt.y, axiscnt.z, leafdim);
      Vector3DI vb = mVoxResMax;
      vb /= Vector3DI(leafdim, leafdim, leafdim);
      long vbrk = vb.x * vb.y * vb.z;                // number of bricks covering bounded world domain
...
void VolumeGVDB::PrepareVDB() {
  ...
  if (mVDBInfo.update) {
    ...
    PUSH_CTX
    cudaCheck(cuMemcpyHtoD(cuVDBInfo, &mVDBInfo, sizeof(VDBInfo)), "VolumeGVDB", "PrepareVDB", "cuMemcpyHtoD",
              "cuVDBInfo", mbDebug);
    POP_CTX
  }
}
void VolumeGVDB::RetrieveVDB() {
  PUSH_CTX
  cudaCheck(cuMemcpyDtoH(&mVDBInfo, cuVDBInfo, sizeof(VDBInfo)), "VolumeGVDB", "RetrieveVDB", "cuMemcpyDtoH",
            "cuVDBInfo", mbDebug);
  POP_CTX
}
void VolumeGVDB::PrepareVDBPartially() {
  ...
  PUSH_CTX
  cudaCheck(cuMemcpyHtoD(cuVDBInfo, &mVDBInfo, sizeof(VDBInfo)), "VolumeGVDB", "PrepareVDBPartially", "cuMemcpyHtoD",
            "cuVDBInfo", mbDebug);
  POP_CTX
}
...
void VolumeGVDB::WriteDepthTexGL(int chan, int glid) {
  ...
  glReadPixels(0, 0, static_cast<GLsizei>(mRenderBuf[chan].stride),
               static_cast<GLsizei>(mRenderBuf[chan].max / mRenderBuf[chan].stride), GL_DEPTH_COMPONENT, GL_FLOAT, 0);
...
void VolumeGVDB::WriteRenderTexGL(int chan, int glid) {
  ...
  if (mRenderBuf[chan].garray == 0) {
    ...
    cudaCheck(
        cuGraphicsGLRegisterImage(&mRenderBuf[chan].grsc, glid, GL_TEXTURE_2D, CU_GRAPHICS_REGISTER_FLAGS_SURFACE_LDST),
        "VolumeGVDB", "WriteRenderTexGL", "cuGraphicsGLRegisterImage", "mRenderBuf", mbDebug);
    cudaCheck(cuGraphicsMapResources(1, &mRenderBuf[chan].grsc, 0), "VolumeGVDB", "WriteRenderTexGL",
              "cuGraphicsMapResources", "mRenderBuf", mbDebug);
    cudaCheck(cuGraphicsSubResourceGetMappedArray(&mRenderBuf[chan].garray, mRenderBuf[chan].grsc, 0, 0), "VolumeGVDB",
              "WriteRenderTexGL", "cuGraphicsSubResourceGetMappedArray", "mRenderBuf", mbDebug);
    cudaCheck(cuGraphicsUnmapResources(1, &mRenderBuf[chan].grsc, 0), "VolumeGVDB", "WriteRenderTexGL",
              "cuGraphicsUnmapResources", "mRenderBuf", mbDebug);
  }
...
void VolumeGVDB::ReadRenderTexGL(int chan, int glid) {
  ...
  if (mRenderBuf[chan].garray == 0) {
    ...
    cudaCheck(
        cuGraphicsGLRegisterImage(&mRenderBuf[chan].grsc, glid, GL_TEXTURE_2D, CU_GRAPHICS_REGISTER_FLAGS_SURFACE_LDST),
        "VolumeGVDB", "ReadRenderTexGL", "cuGraphicsGLRegisterImage ", "mRenderBuf", mbDebug);
    cudaCheck(cuGraphicsMapResources(1, &mRenderBuf[chan].grsc, 0), "VolumeGVDB", "ReadRenderTexGL",
              "cuGraphicsMapResources", "mRenderBuf", mbDebug);
    cudaCheck(cuGraphicsSubResourceGetMappedArray(&mRenderBuf[chan].garray, mRenderBuf[chan].grsc, 0, 0), "VolumeGVDB",
              "ReadRenderTexGL", "cuGraphicsSubResourcesGetMappedArray", "mRenderBuf", mbDebug);
    cudaCheck(cuGraphicsUnmapResources(1, &mRenderBuf[chan].grsc, 0), "VolumeGVDB", "ReadRenderTexGL",
              "cuGraphicsUnmapResources", "mRenderBuf", mbDebug);
  }
  size_t bpp = mRenderBuf[chan].size / mRenderBuf[chan].max;
  size_t width = mRenderBuf[chan].stride * bpp;
...
void VolumeGVDB::ResizeDepthBuf(int chan, int width, int height) {
  ...
  cudaCheck(cuGraphicsGLRegisterBuffer(&mRenderBuf[chan].grsc, mRenderBuf[chan].glid, 0), "VolumeGVDB",
            "ResizeDepthBuf", "cuGraphicsGLRegisterBuffer", "", mbDebug);
  cudaCheck(cuGraphicsMapResources(1, &mRenderBuf[chan].grsc, 0), "VolumeGVDB", "ResizeDepthBuf",
            "cuGraphicsMapResources", "", mbDebug);
  cudaCheck(cuGraphicsResourceGetMappedPointer(&mRenderBuf[chan].gpu, &mRenderBuf[chan].size, mRenderBuf[chan].grsc),
            "VolumeGVDB", "ResizeDepthBuf", "cuGraphicsResourcesGetMappedPointer", "", mbDebug);
  cudaCheck(cuGraphicsUnmapResources(1, &mRenderBuf[chan].grsc, 0), "VolumeGVDB", "ResizeDepthBuf",
            "cuGraphicsUnmapResources", "", mbDebug);
...
void VolumeGVDB::ResizeRenderBuf(int chan, int width, int height, int byteperpix) {
  ...
  if (mRenderBuf[chan].garray != 0x0) {
    cudaCheck(cuGraphicsUnregisterResource(mRenderBuf[chan].grsc), "VolumeGVDB", "ResizeRenderBuf",
              "cuGraphicsUnregisterResource", "", mbDebug);
...
void VolumeGVDB::PrepareRender(int w, int h, char shading) {
  ...
  cudaCheck(cuMemcpyHtoD(cuScnInfo, &mScnInfo, sizeof(ScnInfo)), "VolumeGVDB", "PrepareRender", "cuMemcpyHtoD",
            "cuScnInfo", mbDebug);
...
void VolumeGVDB::RenderKernel(CUfunction user_kernel, uchar chan, uchar rbuf) {
  ...
  cudaCheck(cuLaunchKernel(user_kernel, grid.x, grid.y, 1, block.x, block.y, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "RenderKernel", "cuLaunch", "(user kernel)", mbDebug);
...
void VolumeGVDB::Render(char shading, uchar chan, uchar rbuf) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[kern], grid.x, grid.y, 1, block.x, block.y, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "Render", "cuLaunch", mRendName[shading], mbDebug);
...
void VolumeGVDB::Raytrace(DataPtr rays, uchar chan, char shading, int frame, float bias) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_RAYTRACE], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "Raytrace", "cuLaunch", "FUNC_RAYTRACE", mbDebug);
...
void VolumeGVDB::UpdateApron() {
  for (int n = 0; n < mPool->getNumAtlas(); n++) UpdateApron(n);  // only update marker channel
}
...
void VolumeGVDB::ComputeKernel(CUmodule user_module, CUfunction user_kernel, uchar channel, bool bUpdateApron,
                               bool skipOverAprons) {
...
void VolumeGVDB::Compute(int effect, uchar channel, int num_iterations, Vector3DF parameters, bool bUpdateApron,
                         bool skipOverAprons, float boundval) {
  PERF_PUSH("Compute");
...
//-----------------------------------------------
// Boolean Operations between GVDB volumes
//-----------------------------------------------

void VolumeGVDB::ActivateBricksFrom(VolumeGVDB* other) {
  // Activate bricks in this volume that correspond to bricks in the other volume
  // This is needed for union operations to ensure we have topology for both volumes

  int num_other_nodes = other->getNumUsedNodes(0);  // Level 0 = leaf nodes

  for (int n = 0; n < num_other_nodes; n++) {
    Node* other_node = other->getNodeAtLevel(n, 0);
    if (other_node == nullptr || !other_node->mFlags) continue;

    // Get the world position of this brick
    Vector3DF pos = Vector3DF(other_node->mPos);

    // Activate space at this position in our volume
    bool bNew = false;
    ActivateSpace(pos);
  }
}

void VolumeGVDB::BooleanUnion(uchar dst_chan, uchar srcA_chan, VolumeGVDB* srcB_vol, uchar srcB_chan,
                              bool bUpdateApron) {
  PERF_PUSH("BooleanUnion");

  // For union, we need to ensure topology covers both volumes
  // First, activate any bricks from B that aren't in A
  ActivateBricksFrom(srcB_vol);
  FinishTopology();
  UpdateAtlas();

  // Prepare both volumes' VDB info
  PrepareVDB();
  srcB_vol->PrepareVDB();

  PUSH_CTX

  // Get the second volume's VDBInfo pointer
  cuSecondVDBInfo = srcB_vol->cuVDBInfo;

  // Determine grid and block dims (must match atlas bricks)
  Vector3DI block(8, 8, 8);
  Vector3DI atlasRes = mPool->getAtlasRes(dst_chan);
  Vector3DI grid = (atlasRes + block - Vector3DI(1, 1, 1)) / block;

  void* args[6] = {&cuVDBInfo, &cuSecondVDBInfo, &atlasRes, &srcA_chan, &srcB_chan, &dst_chan};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_BOOLEAN_UNION], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, mStream,
                           args, NULL),
            "VolumeGVDB", "BooleanUnion", "cuLaunch", "FUNC_BOOLEAN_UNION", mbDebug);

  if (bUpdateApron) {
    cudaCheck(cuCtxSynchronize(), "VolumeGVDB", "BooleanUnion", "cuCtxSynchronize", "", mbDebug);
    UpdateApron(dst_chan);
  }
...
void VolumeGVDB::BooleanIntersection(uchar dst_chan, uchar srcA_chan, VolumeGVDB* srcB_vol, uchar srcB_chan,
                                     bool bUpdateApron) {
  PERF_PUSH("BooleanIntersection");

  // For intersection, we only need topology from A (result is subset of A)
  PrepareVDB();
  srcB_vol->PrepareVDB();
  ...
  void* args[6] = {&cuVDBInfo, &cuSecondVDBInfo, &atlasRes, &srcA_chan, &srcB_chan, &dst_chan};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_BOOLEAN_INTERSECT], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0,
                           mStream, args, NULL),
            "VolumeGVDB", "BooleanIntersection", "cuLaunch", "FUNC_BOOLEAN_INTERSECT", mbDebug);

  if (bUpdateApron) {
    cudaCheck(cuCtxSynchronize(), "VolumeGVDB", "BooleanIntersection", "cuCtxSynchronize", "", mbDebug);
    UpdateApron(dst_chan);
  }
...
void VolumeGVDB::BooleanDifference(uchar dst_chan, uchar srcA_chan, VolumeGVDB* srcB_vol, uchar srcB_chan,
                                   bool bUpdateApron) {
  PERF_PUSH("BooleanDifference");

  // For difference A - B, we only need topology from A
  PrepareVDB();
  srcB_vol->PrepareVDB();
  ...
  void* args[6] = {&cuVDBInfo, &cuSecondVDBInfo, &atlasRes, &srcA_chan, &srcB_chan, &dst_chan};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_BOOLEAN_DIFFERENCE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0,
                           mStream, args, NULL),
            "VolumeGVDB", "BooleanDifference", "cuLaunch", "FUNC_BOOLEAN_DIFFERENCE", mbDebug);

  if (bUpdateApron) {
    cudaCheck(cuCtxSynchronize(), "VolumeGVDB", "BooleanDifference", "cuCtxSynchronize", "", mbDebug);
    UpdateApron(dst_chan);
  }
...
void VolumeGVDB::DownsampleCPU(Matrix4F xform, Vector3DI in_res, char in_aux, Vector3DI out_res, Vector3DF out_max,
                               char out_aux, Vector3DF inr, Vector3DF outr) {
  PUSH_CTX
  ...
  void* args[8] = {&in_res, &mAux[in_aux].gpu, &out_res, &out_max, &mAux[out_aux].gpu, &mAux[AUX_MATRIX4F].gpu, &inr,
                   &outr};
  cudaCheck(
      cuLaunchKernel(cuFunc[FUNC_DOWNSAMPLE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
...
Vector3DF VolumeGVDB::Reduction(uchar chan) {
  ...
  cudaCheck(
      cuLaunchKernel(cuFunc[FUNC_REDUCTION], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
...
void VolumeGVDB::Resample(uchar chan, Matrix4F xform, Vector3DI in_res, char in_aux, Vector3DF inr, Vector3DF outr) {
  ...
  cudaCheck(
      cuLaunchKernel(cuFunc[FUNC_RESAMPLE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
...
bool VolumeGVDB::isActive(Vector3DI wpos) { return isActive(wpos, mRoot); }
...
void VolumeGVDB::PrefixSum(CUdeviceptr inArray, CUdeviceptr outArray, int numElem) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], blks1, 1, 1, threads, 1, 1, 0, NULL, argsA, NULL), "VolumeGVDB",
            "PrefixSum", "cuLaunch", "FUNC_PREFIXSUM (in,out->A1)", mbDebug);
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], blks2, 1, 1, threads, 1, 1, 0, NULL, argsB, NULL), "VolumeGVDB",
            "PrefixSum", "cuLaunch", "FUNC_PREFIXSUM (A1,S1->A2)", mbDebug);

  // int blen2 = blks2 * naux;
  void* argsC[5] = {&mAux[AUX_ARRAY2].gpu, &mAux[AUX_SCAN2].gpu, &nptr, &blks2, &zon};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], 1, 1, 1, threads, 1, 1, 0, NULL, argsC, NULL), "VolumeGVDB",
            "PrefixSum", "cuLaunch", "FUNC_PREFIXSUM (A2,S2->null)", mbDebug);

  void* argsD[3] = {&mAux[AUX_SCAN1].gpu, &mAux[AUX_SCAN2].gpu, &blks1};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_PREFIXFIXUP], blks2, 1, 1, threads, 1, 1, 0, NULL, argsD, NULL), "VolumeGVDB",
            "PrefixSum", "cuLaunch", "FUNC_PREFIXFIXUP (S2->S1)", mbDebug);
#endif

  void* argsE[3] = {&outArray, &mAux[AUX_SCAN1].gpu, &numElem};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_PREFIXFIXUP], blks1, 1, 1, threads, 1, 1, 0, NULL, argsE, NULL), "VolumeGVDB",
            "PrefixSum", "cuLaunch", "FUNC_PREFIXFIXUP (S1->out)", mbDebug);
...
char* VolumeGVDB::getDataCPU(DataPtr ptr, int n, int stride) { return (ptr.cpu + n * stride); }
...
void VolumeGVDB::SetDiv(DataPtr div) { mAux[AUX_DIV] = div; }
...
void VolumeGVDB::InsertPointsSubcell(int subcell_size, int num_pnts, float pRadius, Vector3DF trans,
                                     int& pSCPntsLength) {
...
void VolumeGVDB::InsertPointsSubcell_FP16(int subcell_size, int num_pnts, float pRadius, Vector3DF trans,
                                          int& pSCPntsLength) {
  PrepareVDB();
  ...
  cudaCheck(cuMemcpyDtoH(&tmpb, mAux[AUX_SUBCELL_CNT].gpu + (numSCell - 1) * sizeof(int), sizeof(int)), "VolumeGVDB",
            "InsertPointsSubcell_FP16", "cuMemcpyDtoH", "AUX_SUBCELL_CNT", mbDebug);
...
void VolumeGVDB::MapExtraGVDB(int subcell_size) {
  ...
  void* argsSCPos[10] = {&cuVDBInfo,    &numSCellMapping,
                         &SCDim,        &numSCellPerBrick,
                         &subcell_size, &mAux[AUX_SUBCELL_MAPPING].gpu,
                         &cuOBSVDBInfo, &mAux[AUX_SUBCELL_OBS_NID].gpu};
  int pblks = int(numSCellMapping / threads) + 1;
...
void VolumeGVDB::InsertPoints(int num_pnts, Vector3DF trans, bool bPrefix) {
  ...
  void* args[9] = {&cuVDBInfo,
                   ...
                   &mAux[AUX_GRIDCNT].gpu,
                   &trans.x};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_INSERT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "InsertPoints", "cuLaunch", "FUNC_INSERT_POINTS", mbDebug);
  ...
  if (bPrefix) {
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_SORT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
              "InsertPoints", "cuLaunch", "FUNC_SORT_POINTS", mbDebug);
...
void VolumeGVDB::ScalePntPos(int num_pnts, float scale) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_SCALE_PNT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "ScalePntPos", "cuLaunch", "FUNC_SCALE_PNT_POS", mbDebug);

  POP_CTX
}

void VolumeGVDB::GatherDensity(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength,
                               int chanDensity, int chanClr, bool bAccum) {
  ...
  PUSH_CTX
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_GATHER_DENSITY], numSCell, 1, 1, subcell_size, subcell_size, subcell_size, 0,
                           NULL, args, NULL),
...
void VolumeGVDB::GatherLevelSet(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength,
                                int chanDensity, int chanClr, bool bAccum) {
  int num_brick = static_cast<int>(mPool->getPoolUsedCnt(0, 0));
  if (num_brick == 0) return;
  ...
  PUSH_CTX
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_GATHER_LEVELSET], numSCell, 1, 1, subcell_size, subcell_size, subcell_size, 0,
                           NULL, args, NULL),
...
void VolumeGVDB::GatherLevelSet_FP16(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength,
                                     int chanDensity, int chanClr) {
  int num_brick = static_cast<int>(mPool->getPoolUsedCnt(0, 0));
  if (num_brick == 0) return;
  ...
  PUSH_CTX
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_GATHER_LEVELSET_FP16], numSCell, 1, 1, subcell_size, subcell_size, subcell_size,
                           0, NULL, args, NULL),
...
void VolumeGVDB::GetBoundingBox(int num_pnts, Vector3DF pTrans) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_SPLIT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
...
char* VolumeGVDB::GetTestPtr() { return mAux[AUX_TEST].cpu; }
...
void VolumeGVDB::ReadGridVel(int N) {
  ...
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_READ_GRID_VEL], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "ReadGridVel", "cuLaunch", "FUNC_READ_GRID_VEL", mbDebug);
...
void VolumeGVDB::CheckVal(float slice, int chanVx, int chanVy, int chanVz, int chanVxOld, int chanVyOld,
                          int chanVzOld) {
  PUSH_CTX
  ...
  void* args[11] = {&cuVDBInfo,
                    ...
                    &mAux[AUX_OUT2].gpu};
  cudaCheck(
      cuLaunchKernel(cuFunc[FUNC_CHECK_VAL], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
      "VolumeGVDB", "CheckVal", "cuLaunch", "FUNC_CHECK_VAL", mbDebug);
...
void VolumeGVDB::ScatterDensity(int num_pnts, float radius, float amp, Vector3DF trans, bool expand, bool avgColor) {
  ...
  void* args[13] = {&num_pnts,
                    ...
                    &expand,
                    &mAux[AUX_COLAVG].gpu};
  cudaCheck(cuLaunchKernel(cuFunc[FUNC_SCATTER_DENSITY], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "ScatterPointDensity", "cuLaunch", "FUNC_SCATTER_DENSITY", mbDebug);

  if (mAux[AUX_PNTCLR].gpu != NULL && avgColor) {
    int threads_avgcol = 256;
    int pblks_avgcol = int(num_voxels / threads_avgcol) + 1;
    void* args_avgcol[2] = {&num_voxels, &mAux[AUX_COLAVG].gpu};
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_SCATTER_AVG_COL], pblks_avgcol, 1, 1, threads_avgcol, 1, 1, 0, NULL,
                             args_avgcol, NULL),
...
//
void VolumeGVDB::ConvertAndTransform(DataPtr& psrc, char psrcbits, DataPtr& pdest, char pdestbits, int num_pnts,
                                     Vector3DF wMin, Vector3DF wDelta, Vector3DF trans, Vector3DF scal) {
  PUSH_CTX

  int threads = 512;
  int pblks = int(num_pnts / threads) + 1;
  void* args[9] = {&num_pnts, &psrc.gpu, &psrcbits, &pdest.gpu, &pdestbits, &wMin, &wDelta, &trans, &scal};

  cudaCheck(cuLaunchKernel(cuFunc[FUNC_CONV_AND_XFORM], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB",
            "ConvertAndTransform", "cuLaunch", "FUNC_CONV_AND_XFORM", mbDebug);

  POP_CTX
}

void VolumeGVDB::AddSupportVoxel(int num_pnts, float radius, float offset, float amp, Vector3DF trans, bool expand,
                                 bool avgColor) {
  PrepareVDB();
...
Vector3DF VolumeGVDB::getWorldMin() { return mObjMin; }
Vector3DF VolumeGVDB::getWorldMax() { return mObjMax; }
