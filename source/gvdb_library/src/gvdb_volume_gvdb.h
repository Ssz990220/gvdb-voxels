...
// Boolean Operations
#define FUNC_BOOLEAN_UNION 120
#define FUNC_BOOLEAN_INTERSECT 121
#define FUNC_BOOLEAN_DIFFERENCE 122
...
class GVDB_API VolumeGVDB : public VolumeBase {
 public:
  ...
  void Compute(int effect, uchar channel, int num_iterations, Vector3DF parameters, bool bUpdateApron,
               bool skipOverAprons, float boundval = 0.0f);
  ...
  void ComputeKernel(CUmodule user_module, CUfunction user_kernel, uchar channel, bool bUpdateApron,
                     bool skipOverAprons);
  ...
  void DownsampleCPU(Matrix4F xform, Vector3DI in_res, char in_aux, Vector3DI out_res, Vector3DF out_max, char out_aux,
                     Vector3DF inr, Vector3DF outr);
  ...
  void AddChannel(uchar chan, int dt, int apron, int filter = F_LINEAR, int border = F_CLAMP,
  ...
  slong ActivateSpace(slong nodeid, Vector3DI pos, bool& bNew, slong stopnode = ID_UNDEFL,
  ...
  void ActivateIncreBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos,
                              bool bAccum = false);
  ...
  int VoxelizeNode(Node* node, uchar chan, Matrix4F* xform, float bdiv, float val_surf, float val_inside,
                   float vthresh = 0.0);
  ...
  void AddSupportVoxel(int num_pnts, float radius, float offset, float amp, Vector3DF trans, bool expand = true,
                       bool avgColor = false);
  ...
  void ScatterDensity(int num_pnts, float radius, float amp, Vector3DF trans, bool expand = true,
                      bool avgColor = false);
  void GatherDensity(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength, int chanDensity,
                     int chanClr, bool bAccum = false);
  void GatherLevelSet(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength,
                      int chanLevelset, int chanClr, bool bAccum = false);
  void GatherLevelSet_FP16(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength,
                           int chanLevelSet, int chanClr);
  void ConvertAndTransform(DataPtr& psrc, char psrcbits, DataPtr& pdest, char pdestbits, int num_pnts, Vector3DF wMin,
                           Vector3DF wMax, Vector3DF trans, Vector3DF scal);
  ...
  // Performs union: dst = A union B (max of values)
  // srcA_chan: channel from this volume, srcB_vol: other volume, srcB_chan: channel from other volume
  // dst_chan: destination channel in this volume
  void BooleanUnion(uchar dst_chan, uchar srcA_chan, VolumeGVDB* srcB_vol, uchar srcB_chan, bool bUpdateApron = true);

  // Performs intersection: dst = A intersect B (min of values, or 0 if either is 0)
  void BooleanIntersection(uchar dst_chan, uchar srcA_chan, VolumeGVDB* srcB_vol, uchar srcB_chan,
                           bool bUpdateApron = true);

  // Performs difference: dst = A - B (A where B is 0, 0 where B > 0)
  void BooleanDifference(uchar dst_chan, uchar srcA_chan, VolumeGVDB* srcB_vol, uchar srcB_chan,
                         bool bUpdateApron = true);

  // Activate bricks in this volume that correspond to bricks in another volume
  // Used to prepare topology for boolean operations
  void ActivateBricksFrom(VolumeGVDB* other);
...
#define B6(n) B4(n), B4(n + 1), B4(n + 1), B4(n + 2)
  ...
  inline uint64 firstBitOn(uint32 v) {
    assert(v);
    static const byte DeBruijnBitPos[32] = {0,  1,  28, 2,  29, 14, 24, 3, 30, 22, 20, 15, 25, 17, 4,  8,
  ...
  inline uint64 lastBitOn(uint32 v) {
    static const byte DeBruijn[32] = {0, 9,  1,  10, 13, 21, 2,  29, 11, 14, 16, 18, 22, 25, 3, 30,
 ...
 protected:
  ...
  CUdeviceptr cuSecondVDBInfo;  // Non-owning pointer to VDBInfo used for boolean operations
