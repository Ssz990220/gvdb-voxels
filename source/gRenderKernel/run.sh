#!/bin/bash
# Wrapper script to run gRenderKernel with correct working directory for data files
...
# Link PTX files from gvdb_library
ln -sf "$RUNFILES_DIR/source/gvdb_library"/*.ptx "$WORK_DIR/" 2>/dev/null || true

# Link sample-specific PTX file
ln -sf "$RUNFILES_DIR/source/gRenderKernel"/*.ptx "$WORK_DIR/" 2>/dev/null || true
...
exec "$RUNFILES_DIR/source/gRenderKernel/gRenderKernel" "$@"
